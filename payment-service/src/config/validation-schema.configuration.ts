...
export const validationSchemaConfiguration = Joi.object({
  NODE_ENV: Joi.string().valid(EnvNamesEnums.DEVELOPMENT, EnvNamesEnums.PRODUCTION, EnvNamesEnums.TEST),
  MONGO_URI_LOCAL: Joi.string().uri().min(10).max(23).required(),
  ATLAS_URI: Joi.string().uri().min(10).max(63).required(),
  TEST_DATABASE: Joi.string().pattern(new RegExp('^[-a-zA-Z0-9]{7}$')).required(),
  DEV_DATABASE: Joi.string().pattern(new RegExp('^[-a-zA-Z0-9]{16}$')).required(),
  PROD_NEST_DATABASE: Joi.string().pattern(new RegExp('^[-a-zA-Z0-9]{15}$')).required(),
  ...
  ACCESS_SECRET_KEY: Joi.string().pattern(new RegExp('^[a-zA-Z0-9]{15,100}$')).required(),
  REFRESH_SECRET_KEY: Joi.string().pattern(new RegExp('^[a-zA-Z0-9]{15,100}$')).required(),
  EXP_ACC_TIME: Joi.string().pattern(new RegExp('^[a-zA-Z0-9]{1,10}$')).required(),
  EXP_REF_TIME: Joi.string().pattern(new RegExp('^[a-zA-Z0-9]{1,10}$')).required(),
  ...
  DATABASE_URL: Joi.string()
    .min(174)
    .max(174)
    .pattern(new RegExp('^postgres:\\/\\/[a-zA-Z0-9]+:[\\w\\W]+@[a-zA-Z0-9.-]+:[0-9]+\\/[a-zA-Z0-9]+$'))
    .required(),
  PG_HEROKU_USER_NAME: Joi.string().min(14).max(14).pattern(new RegExp('^[a-zA-Z0-9]')).required(),
  PG_HEROKU_NAME_DATABASE: Joi.string().pattern(new RegExp('^[-a-zA-Z0-9]{13}$')).required(),
  PG_HEROKU_USER_PASSWORD: Joi.string().min(65).max(65).required(),
  PG_DOMAIN_HEROKU: Joi.string()
    .pattern(new RegExp('^https:\\/\\/([a-z0-9]+(-[a-z0-9]+)*\\.)+[a-z]{2,}(:[0-9]+)?(\\/.*)?$'))
    .required(),
  PG_LOCAL_USER_NAME: Joi.string().min(2).max(20).pattern(new RegExp('^[a-zA-Z0-9]')).required(),
  ...
  messages: {
    ...
    'string.min': 'The {{#label}} environment variable must be at least {{#limit}} characters long.',
    'string.max': 'The {{#label}} environment variable must be at most {{#limit}} characters long.',
    'number.base': 'The {{#label}} environment variable must be a number.',
    'number.default': 'Invalid value provided for the {{#label}} environment variable.',
    'string.uri': 'The {{#label}} environment variable must be a valid URI.',
    'string.email': 'The {{#label}} environment variable must be a valid email address.',
    'number.min': 'The {{#label}} environment variable must be at least {{#limit}}.',
    'number.max': 'The {{#label}} environment variable must be at most {{#limit}}.',
